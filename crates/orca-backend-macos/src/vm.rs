...
provision:
  - mode: system
    script: |
      ...
      # caches the state and surfaces "IPv4 forwarding is disabled. Networking
      # will not work." on `docker compose up` for its whole lifetime — only a
      # restart makes it re-read. The sysctl.d drop-in makes the setting survive
      # reboots (applied early by systemd-sysctl, before docker); when it was
      # off we apply it now and bounce dockerd so the running daemon re-reads.
